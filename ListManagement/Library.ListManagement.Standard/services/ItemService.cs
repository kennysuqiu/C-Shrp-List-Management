...
namespace ListManagement.services
{
    public class ItemService
    {
        ...
        public async Task<AppointmentDTO> AddApt(ItemDTO i)
        {
            var toDoStr = await new WebRequestHandler().Post("http://localhost:7020/Appointment/AddOrUpdate", i);
            return JsonConvert.DeserializeObject<AppointmentDTO>(toDoStr);
        }

        public async Task<ToDoDTO> RemoveToDo(int id)
        {
        ...
        public async Task<AppointmentDTO> RemoveApt(int id)
        {
            var deletedToDoStr = await new WebRequestHandler().Post("http://localhost:7020/Appointment/Delete", new DeleteItemDTO { IdToDelete = id });
            return JsonConvert.DeserializeObject<AppointmentDTO>(deletedToDoStr);
        }
